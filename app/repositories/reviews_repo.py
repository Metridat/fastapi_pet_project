...
class ReviewsRepo:
    ...
    async def get_reviews(self, active: bool = True):
        reviews = await self.db.scalars(select(ReviewModel)
                                        .join(ProductModel)
                                        .join(CategoryModel)
                                        .where(
    ...
    async def get_review(self, review_id: int, active: bool = True):
        return await self.db.scalar(select(ReviewModel)
                                    .join(ProductModel)
                                    .join(CategoryModel)
                                    .where(
    ...
    async def get_review_product(self, product_id: int):
        reviews_for_products = await self.db.scalars(select(ReviewModel)
                                                     .join(ProductModel)
                                                     .join(CategoryModel)
                                                     .where(
    ...
    async def get_review_user(self, buyer_id: int, product_id: int):
        return await self.db.scalar(select(ReviewModel).where(
